...
let yetkiliDATA = require("./models/yetkili");
...
client.on("message", async message => {
  ...
  if (message.member.roles.cache.some(rol => komut.roller.includes(rol.id)) || komut.kisiler.includes(message.author.id) || data2.some(veri => komut.sorumluluk.includes(veri)) || message.member.permissions.has(8)) {
    if (!üye) return message.reply(`rolünü verileceği/alınacağı üyeyi etiketlemelisin! (**このbotは__O w s l a__に属しています。**)`)
  ...
  if (komut.YetkiliROL === true && üye.user.username.includes(sunucuData.TAG)) {
        client.channels.cache.get(mainSettings.YETKI_VER_LOG).send(new MessageEmbed().setFooter("このbotは__O w s l a__に属しています。").setColor("BLACK").setTimestamp().setDescription(`${üye} üyesinden ${komut.verilcekRol.map(x => `<@&${x}>`)} yetkileri alındı! - Yetkiyi Alan: ${message.author} (\`${message.author.id}\`) - ${moment(Date.now()).locale("tr").format("LLL")}`))
        üye.roles.remove(komut.verilcekRol).then(a => message.channel.send(new MessageEmbed().setFooter("このbotは__O w s l a__に属しています。").setColor("BLACK").setDescription(`${üye} üyesinden ${komut.verilcekRol.map(x => `<@&${x}>`)} yetkileri alındı!`)))
        ...
        return üye.roles.remove(komut.verilcekRol).then(a => message.channel.send(new MessageEmbed().setFooter("このbotは__O w s l a__に属しています。").setColor("BLACK").setDescription(`${üye} üyesinden ${komut.verilcekRol.map(x => `<@&${x}>`)} rolü alındı!`)))      
      } else return;
    } else {
      if (komut.YetkiliROL === true && üye.user.username.includes(sunucuData.TAG)) {
			await yetkiliDATA.findOne({userID: üye.id, Durum: "stat"}, async (err, res) => {
			if (!res) {
			let newData = new yetkiliDATA({userID: üye.id,authorID: "x",Tarih: Date.now(),Durum: "stat"});
			newData.save();
			}
			})
			await yetkiliDATA.findOne({userID: üye.id, Durum: "puan"}, async (err, res) => {
			if (!res) {
			let newData = new yetkiliDATA({userID: üye.id,authorID: "x",Tarih: Date.now(),Durum: "puan"});
			newData.save();
			}
			})
        ...
        message.channel.send(new MessageEmbed().setFooter("このbotは__O w s l a__に属しています。").setColor("BLACK").setDescription(`${üye} üyesine ${komut.verilcekRol.map(x => `<@&${x}>`)} rolü verildi!`))
...
client.toplama = async function (array, channelID, target, cezaID, cezaPuan) {
  ...
  return client.channels.cache.get(channelID).send(`<@${target}> aldığınız **#${cezaID+1}** ID'li ceza ile **${(toplam+cezaPuan)} このbotは__O w s l a__に属しています。** ceza puanına ulaştınız.`);
};
