...
module.exports.run = async (client, message, args, durum, kanal) => {
    ...
    if (message.member.permissions.has(8) || durum) {
        ...
        let uyarı = await uyarıData.find({userID: target.id}) || [];
        let puan = await xpData.findOne({userID: target.id}) || {currentXP: 0};

        let yetkiler = kanallar.PuanRolSystem;
        let ekPuan = puan.currentXP;
		let ceza_puan = uyarı.length > 3 ? uyarı.length > 3 ? (uyarı.length-3) * (250) : 0 : 0


        let pubPuan = target.roles.cache.some(rol => ["838817061362073690"].includes(rol.id)) ? kanallar.PublicKanallar.Puan*1.2 : kanallar.PublicKanallar.Puan;
        ...
        let streamPuan = target.roles.cache.some(rol => ["838817061303877719"].includes(rol.id)) ? kanallar.StreamKanallar.Puan*1.2 : kanallar.StreamKanallar.Puan;
        let secretPuan = target.roles.cache.some(rol => kanallar.SecretKanallar.Rol.includes(rol.id)) ? 2 : kanallar.SecretKanallar.Puan;
        let mesajPuan = target.roles.cache.some(rol => ["838817061387894787"].includes(rol.id)) ? kanallar.MesajKanallar.Puan*1.2 : kanallar.MesajKanallar.Puan;
		...
		let yetkili_alim = arr.filter(x => x.Author == target.id).length
		...
		parseInt((mesaj * mesajPuan).toFixed(0)) +
		...
		parseInt((yetkili_alim * 10)) +
		parseInt((taglı * taglıPuan)) +
		parseInt((invite * invitePuan)) - Number(ceza_puan) + Number(data.EtkinlikPuan)
...
**Notlar**
Şuanda \`${uyarı.length}\` uyarın görünmekte eğer uyarı sayın 3'ün üzerine çıkarsa aldığın her uyarı kalıcı olarak sana \`-250\` ve katı şeklinde puan ekleyecektir. (\`Ceza Puan: ${uyarı.length > 3 ? uyarı.length > 3 ? (uyarı.length-3) * (-250) : 0 : 0}\`)

**Bilgiler**
${statemoji} Toplam Puanınız: \`${(totalpoints+parseInt(ekPuan))}\`
${statemoji} Görev Puanınız: \`${ekPuan}\`
${statemoji} Ceza Puanınız: \`${uyarı.length > 3 ? uyarı.length > 3 ? (uyarı.length-3) * (-250) : 0 : 0}\`

**Ses Bilgileri**
${statemoji} Public Kanallar: \`${client.convertDuration(pubOda)} (${parseInt(pubOda/(1000 * 60 * 60 * 1) * pubPuan)} puan)\`
${statemoji} Streamer Odaları: \`${client.convertDuration(stream)} (${parseInt(stream/(1000 * 60 * 60 * 1) * streamPuan)} puan)\`
${statemoji} Oyun Odaları: \`${client.convertDuration(oyunodalar)} (${parseInt(oyunodalar/(1000 * 60 * 60 * 1) * oyunPuan)} puan)\`
${statemoji} Private Odaları: \`${client.convertDuration(secret)} (${parseInt(secret/(1000 * 60 * 60 * 1) * secretPuan)} puan)\`
${statemoji} Alone Odaları: \`${client.convertDuration(alone)} (${parseInt(alone/(1000 * 60 * 60 * 1) * alonePuan)} puan)\` ${kanallar.AloneKanallar.Id.length > 0 ? "" : "(**Kapalı**)"}
${statemoji} Müzik Odaları: \`${client.convertDuration(music)} (${parseInt(music/(1000 * 60 * 60 * 1) * musicPuan)} puan)\` ${kanallar.Müzik.Id.length > 0 ? "" : "(**Kapalı**)"}
${statemoji} Sleep Odaları: \`${client.convertDuration(sleeping)} (${parseInt(sleeping/(1000 * 60 * 60 * 1) * sleepPuan)} puan)\`
${statemoji} Terapi Odaları: \`${client.convertDuration(terapi)} (${parseInt(terapi/(1000 * 60 * 60 * 1) * terapipuan)} puan)\` ${kanallar.TerapiKanallar.Id.length > 0 ? "" : "(**Kapalı**)"}
${statemoji} Sorun Çözme Odaları: \`${client.convertDuration(sçözme)} (${parseInt(sçözme/(1000 * 60 * 60 * 1) * sorunçözmepuan)} puan)\`
...
`)
.addField("**Mesaj Bilgileri**", `${statemoji} Genel Mesaj: \`${mesaj} (${(mesaj*mesajPuan).toFixed(0)} puan)\``)
.addField("Diğer Bilgiler", `
${statemoji} Yetkili Alım Miktar: \`${yetkili_alim} (${yetkili_alim*10} puan)\`
...
${progressBar(mission.Mission.MISSION == "ses" ? mission.Check/(1000*60) : mission.Check, mission.Mission.MISSION == "ses" ? mission.Mission.AMOUNT/(1000*60) : mission.Mission.AMOUNT, 8)} \`${mission.Mission.MISSION == "ses" ? `${(mission.Check/(1000*60)).toFixed(0)} / ${(mission.Mission.AMOUNT/(1000*60)).toFixed(0)}` : `${(mission.Check).toFixed(0)} / ${(mission.Mission.AMOUNT).toFixed(0)}`}\`

Bu görevi tamamladığın zaman **Ek Puan** kazanacaksın.
...
${progressBar(totalpoints+parseInt(ekPuan), y.PUAN, 8)}  \`${totalpoints+parseInt(ekPuan)} / ${y.PUAN}\`
...
Şuan <@&${y.ROLE_1}> rolündesiniz. <@&${y.ROLE_2}> rolüne ulaşmak için **${Number(y.PUAN-(totalpoints+parseInt(ekPuan)).toFixed(0))}** \`Puan\` kazanmanız gerekiyor\n─────────────────────` : ""}`) : "Üzerinde bir rol olmadığı için yükselme tablosunu gösteremiyorum."}`)
                ...
                if (kanallar.AutoRankUP.Type == true) {
                    ...
                    yetkiler.slice(0, i).filter(user => target.roles.cache.get(user.ROLE_1)).map(async user => {
                        if ((totalpoints + ekPuan) - (uyarı.length > 3 ? (uyarı.length - 3) * (-250) : 0) >= user.PUAN) {
