...
if (pub.includes(channel) || reg.includes(channel)) {
  hanedan.findOne({userID: id, guildID: client.ayarlar.sunucuId}, (err, data1) => {
    if (data1) {
...
Stream Kanallar (saat) = 35 Puan
AFK Kanallar (saat) = 15 Puan
Kayıt Kanalları (saat) = 25 Puan
*/
	if (pub.includes(channel)) {
		Stat.updateOne({userID: id,guildID: client.ayarlar.sunucuId}, {$inc: {coin: 0.000005555555555555556,EtkinlikPuan: Math.abs(value/(1000 * 60 * 60) * 35)}}, {upsert: true}).exec();	
	}
	if (streamer.includes(channel)) {
		Stat.updateOne({userID: id,guildID: client.ayarlar.sunucuId}, {$inc: {coin: 0.000005555555555555556,EtkinlikPuan: Math.abs(value/(1000 * 60 * 60) * 35)}}, {upsert: true}).exec();	
	}
	if (kayit.includes(channel)) {
		Stat.updateOne({userID: id,guildID: client.ayarlar.sunucuId}, {$inc: {EtkinlikPuan: Math.abs(value/(1000 * 60 * 60) * 25)}}, {upsert: true}).exec();	
	}	
	if (channel == sleep) {
		Stat.updateOne({userID: id,guildID: client.ayarlar.sunucuId}, {$inc: {EtkinlikPuan: Math.abs(value/(1000 * 60 * 60) * 15)}}, {upsert: true}).exec();	
	}
      ...
      Stat.updateOne({
        ...
        $inc: {
          VipActivites: Math.abs(value/(1000*60*60*20)),
